...
from plugins.megatron.tokenizer import build_tokenizer
#import indexed_dataset
from plugins.megatron.indexed_dataset import IndexedDatasetBuilder, DType
    ...
    def process_json_file(self, file_name): # 如果是多分区split，则将其先分成n个partition，然后每个partition处理成_ss，然后将_ss再encode
        ...
        for key in self.args.json_keys:
            ...
            builders[key] = IndexedDatasetBuilder(
                output_bin_files[key],
                dtype=DType.optimal_dtype(tokenizer.vocab_size), # cardinality < 65500
            )
...
def process_data(input, output_prefix, tokenizer_type, vocab_file, jsonl_keys=["text"], split_sentences=False, keep_newlines=False, tokenizer_model=None, vocab_size=786, merge_file=None, workers=6, max_processes=6, log_interval=1000, append_eod=True, lang='english', partitions=1, merge_partitions=True, keep_sequential_samples=False):
    ...
    supported_tokenizers = [
        ...
        'GPT2BPETokenizer', 'SentencePieceTokenizer',
        'GPTSentencePieceTokenizer', 'Llama2Tokenizer',
        'RWKVTokenizer', 'HFTokenizer', 'NullTokenizer'
    ]
...
if __name__ == '__main__':
    ...
    #process_data("/mnt/data/Codes/json2binidx_tool/test_data","./data/test","SentencePieceTokenizer",vocab_file="/mnt/data/Codes/json2binidx_tool/megatron/tokenizer.model")
    process_data(inputpath, output_prefix, tokenizer_type, vocab_file)
    end_time = time.time()
