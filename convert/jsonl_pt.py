...
def process_jsonl(file_chunk, output_directory, text_key, sql_alchemy_path, global_index_start):
    # 获取数据库引擎
    engine = get_engine(sql_alchemy_path)
...
def save_jsonl_to_jsonl(dataset_directory: str, output_directory: str, text_key: str, settings):
    ...
    file_chunks = [jsonl_files[i::workers] for i in range(workers)]

    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = []
        for i, file_chunk in enumerate(file_chunks):
            global_index_start = math.floor(i * 65536 / workers)
            futures.append(executor.submit(process_jsonl, file_chunk, output_directory, text_key, sql_alchemy_path, global_index_start))
