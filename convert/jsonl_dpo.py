...
from plugins.clean_text_line import clean_text_line_sft

from sqlalchemy.exc import SQLAlchemyError
from sqlalchemy.orm import sessionmaker
from sqlalchemy import create_engine
from sqlalchemy import select
from plugins.sql import TextLine_dpo_test
...
def is_first_duplicate(role, content, session):
    query = select(TextLine_dpo_test.line_id).where(
        TextLine_dpo_test.role == role,
        TextLine_dpo_test.content == content,
        TextLine_dpo_test.frontID.is_(None)
    )
    result = session.execute(query).scalar()
    return str(result) if result else None

def is_subsequent_duplicate(role, content, parent_line_id, score, session):
    query = select(TextLine_dpo_test.line_id).where(
        TextLine_dpo_test.role == role,
        TextLine_dpo_test.content == content,
        TextLine_dpo_test.frontID == parent_line_id,
        TextLine_dpo_test.score == score
    )
    result = session.execute(query).scalar()
    return str(result) if result else None
...
def process_json_data(json_data, base_output_name, sql_alchemy_path, global_index_start, current_line_number):
    ...
    first_message['role'] = clean_text_line_sft(first_message['role'])
    first_message['content'] = clean_text_line_sft(first_message['content'])
    ...
    if first_duplicate:
        first_message_line_id = str(first_duplicate)

    else:
        first_message_line_id = generate_line_id(base_output_name, global_index_start, current_line_number.value)
        ...
        store_text_line(session, first_message_line_id, first_message['role'], first_message['content'], None, first_message_line_id, 'false', first_message['score'], first_message['messages_score'], language, industry_subject, keywords, text_complexity)
        #print("finished")
        last_inserted_id = first_message_line_id
        current_line_number.value += 1  # 直接增加值

    for i in range(1, len(messages)):
        message = messages[i]
        role = clean_text_line_sft(message['role'])
        content = clean_text_line_sft(message['content'])
        ...
        duplicate_id = is_subsequent_duplicate(role, content, first_message_line_id, score, session)
        if duplicate_id:
            #last_inserted_id = str(duplicate_id)
            continue

        frontID = first_message_line_id
        indexID = first_message_line_id
...
def process_json(file_chunk, output_directory, dataset_type, sql_alchemy_path, global_index_start, current_line_number):
    ...
    for file_path in file_chunk:
        ...
        if dataset_type == "dpo":
            multi_round_query = f"""
            SELECT line_id, role, content, frontID, indexID, end, score
            FROM text_line_dpo_test
            ...
            def build_list_representation(tree, root_nodes):
                ...
                for root_id in root_nodes:
                    root_node = tree[root_id]['node']
                    root_thread = [root_node]
                    children = tree[root_id]['children']

                    while children:
                        next_children = []
                        for child_id in children:
                            child_node = tree[child_id]['node']
                            root_thread.append(child_node)
                            next_children.extend(tree[child_id]['children'])
                        children = next_children

                    result.append(root_thread)
            ...
            for lst in list_representation:
                # Combine responses with the same frontID
                combined_responses = []
                current_response = lst[0]
                for item in lst[1:]:
                    if item['frontID'] == current_response['line_id']:
                        combined_responses.append(item)
                    else:
                        combined_responses = [item]
                        current_response = item

                line = json.dumps({'messages': [lst[0], *combined_responses]}, ensure_ascii=False)
